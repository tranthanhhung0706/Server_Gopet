                ...
                if (index >= 0 && listItemNormal.Count > index)
                {
                    ...
                    switch (itemSelect.getTemp().getType())
                    {
                        ...
                        case GopetManager.ITEM_ADMIN:
                            {
                                // Trước đây khoá cứng chức năng này (redDialog "tạm khoá"). Giờ: chỉ tài khoản
                                // có cờ admin mới thấy ô nhập mã (App.config -> AdminItemUnlockKey, tách riêng
                                // với ApiSecretKey của API buff_gopet), nhập đúng mới mở MENU_SELECT_ITEM_ADMIN
                                // — xem case INPUT_DIALOG_ITEM_ADMIN_UNLOCK trong MenuController.inputDialog.cs.
                                // Non-admin: chỉ deny, KHÔNG tự ban (bản rất cũ từng ban vĩnh viễn ở đây — bỏ vì
                                // rủi ro ban nhầm nếu có bug/exploit khác khiến item này lọt vào tay non-admin).
                                if (player.checkIsAdmin())
                                {
                                    ...
                                    player.controller.showInputDialog(INPUT_DIALOG_ITEM_ADMIN_UNLOCK, "Nhập mã mở khoá Administrator Item", new String[] { " " }, new sbyte[] { 2 });
                                }
                                else
                                {
